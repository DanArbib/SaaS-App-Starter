...
<template>
  <aside
    class="absolute left-0 top-0 z-9999 flex h-screen w-72.5 flex-col overflow-y-hidden  bg-sidebarlight duration-300 ease-linear border-x-[1px] border-stroke dark:bg-boxdark lg:static lg:translate-x-0 dark:border-strokedark"
    :class="{
  ...
  >
    <!-- SIDEBAR HEADER -->
    <div class="flex items-center justify-between p-2 max-h-[65px]">
    ...
    <div class="flex flex-col flex-grow overflow-y-auto duration-300 ease-linear">
      <!-- Sidebar Menu -->
      <nav class="mt-5 py-4 px-4 lg:mt-9 lg:px-6">
        <template v-for="menuGroup in menuGroups" :key="menuGroup.name">
          <div>
            <h3 class="mb-4 ml-4 text-sm font-medium text-body2 dark:text-bodydark2">{{ menuGroup.name }}</h3>
      ...
      <!-- Promo Box -->
      <div class="mb-12 mt-auto mx-auto w-full max-w-60 rounded-2xl border border-strokedark bg-boxdark-2 py-6 px-4 text-center shadow-default">
        <h3 class="mb-1 font-semibold text-white">Upgrade to Pro</h3>
        <p class="mb-2 text-xs text-whiter">Get All Elements</p>

        <button @click="router.push({ name: 'subscribe' })" class="mt-4 text-white w-full text-base p-2 bg-primary hover:bg-primary/90 rounded-lg">
          Purchase Now
        </button>
      </div>
      <!-- Promo Box -->
